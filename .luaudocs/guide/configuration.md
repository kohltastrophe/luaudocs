...
| Path                                               | Owner                                                                               |
| :------------------------------------------------- | :---------------------------------------------------------------------------------- |
| `guide/`, `public/`, `custom.css`                  | Yours. Written once by `init`, never touched again.                                 |
| `index.md`                                         | Yours, once it stops carrying the `# Generated by luaudocs` marker (details below). |
...
If `luaudocs.toml` and `custom.css` cannot express what your docs need, please open an [issue](https://github.com/kohltastrophe/luaudocs/issues) rather than attempting workarounds inside `api/` or `.vitepress/`, which future builds will overwrite.
:::
...
The pages sit under one **Guide** heading. When `guide/` contains only subfolders, the top-level Guide heading is omitted and each folder becomes its own distinct section. A section header has no toggle unless its [group settings](#group-settings) specify `collapsed` or `collapsible`.
