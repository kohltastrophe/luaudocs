...
LuauDocs is built in two distinct stages connected by a JSON contract: a Luau extractor (running on [Lute](https://github.com/luau-lang/lute)) that reads a library and emits a doc model, and a TypeScript renderer that turns the model into a VitePress site. "The ladder" is the validation sequence detailed below, ordered from fastest to slowest.

Bug reports and pull requests are both welcome. Open an issue first for anything larger than a bug fix. Otherwise: branch, make the ladder green, and describe what changed and why. Any pull request touching the extractor or a fixture must include the regenerated captures (`test/fixtures/docmodel-*.json`, via `bun run regen:docmodels`) in the same commit.
...
That runs the `prepare` hook (`npm run generate && tsup`), populating `src/generated/` and `extractor/generated/`, and building `dist/`. There is no toolchain setup step: the [Lute](https://github.com/luau-lang/lute) binary used by the extractor is pinned in `src/lute.ts` and downloaded into a per-user cache the first time it is spawned, so `test:lute`, `test:e2e`, and `regen:docmodels` all reach the same binary with no setup. Setting `LUAUDOCS_LUTE` overrides the pinned binary with a custom build, allowing you to test against unreleased Lute builds without modifying the pin.

## The ladder

CI runs these in order, on Linux and Windows. Locally, following this sequence surfaces failures quickly by running the fastest checks first:
...
dependencies only, ensuring any packaging issues masked by the local checkout
still fail CI.
...
toward that number, and why process-boundary files are excluded, is documented
in `vitest.config.ts`.

Targeted commands for local iteration:
...
Formatting is enforced: `bun run format` (Prettier for TypeScript) and `bun run format:luau` (StyLua for `extractor/` and `examples/`). Markdown is excluded from both.

## Inspecting extractor output

`luaudocs build --model <file>` exports the doc model rendered by the build, which is the supported approach and sufficient for answering most questions about what the extractor observed. To inspect what the extractor produces for a project without a docs directory or configuration, run the extractor directly on Lute. The resulting JSON is emitted to stdout and can be piped into `jq`:
...
**Extractor** (`extractor/`, 17 files, entry `main.luau`, spawned as a subprocess by `src/extract.ts`). The stages, in order: `parse` (the one module that touches Lute's syntax tree) → `comments` → `surface` (symbolic evaluation down to the module's return value) → `requires` + `graph` → `tags` + `segs` (signature display segments) + `detect` (behavioral badges: yields and realm inferred from function bodies, and property realms read from guarding conditions) + `infer` (the type a value implies, for members declaring none) → `model/` (two passes: `build` assembles each module in isolation, and `finalize` runs the whole-model steps) → `emit` (deterministic JSON with sorted keys).

**Renderer** (`src/`, 15 flat files): `cli`, `config`, `extract` + `docmodel` + `lute`, `build`, `pages` + `nav`, `render`, `site`, `llms`, `init` + `moonwave`, `dev`, and `markdown`. `runBuild` (`src/build.ts`) is the single extract → render → write sequence shared by `build` and `dev` (preventing drift), and it enforces the write contract below.

Every module opens with a block comment stating what it owns; read those rather than a list here.

In `luaudocs.toml`, `[source] entries = []` designates a project with no modules rather than a special mode: the extractor is skipped, and commands execute their standard pipeline.

`nav.ts` mirrors the **public access path** (the expression a caller writes to reach a module), and only modules that are not otherwise exposed fall back to the instance hierarchy (the Rojo tree or the folder layout). It accepts no configuration overrides, so maintaining accurate structure in code is essential.

## Invariants

These design invariants protect against subtle breakage from otherwise reasonable-looking changes.

- **Schema version.** The doc-model shape is documented in the header of `extractor/model/init.luau` and declared in `src/docmodel.ts`, fixed at 1 and evolving in place on both sides during pre-1.0. There is deliberately no runtime field validator: the specs and `regen:docmodels --check` cover what the extractor emits, so `parseDocModel` checks only what a _user_ can trigger (bad JSON, a stale model, or the wrong file).
...
- **Ownership is by directory.** Do not write a generated file anywhere new without deciding which side of this line it lives on. The write contract lives in `syncDir` (`src/build.ts`): `<docs>/api/` and `<docs>/.vitepress/` are synchronized to contain exactly the emitted set, with everything else under them deleted (VitePress's own `dist/`, `cache/`, and config timestamps are exempt); outside those two directories, the tool deletes nothing. At the docs root it overwrites exactly `changelog.md`, `public/llms*.txt`, `package.json`, and (only while it carries the `# Generated by luaudocs` marker) `index.md`. The marker is the ownership handshake (`isGeneratedFile` in `src/pages.ts`): deleting the line is how a user claims a file, and `init` on its side overwrites a still-marked file but never a claimed one. `.gitignore` is created only when missing.
- **Extraction errors never delete pages.** When the model carries any `error` diagnostic, the build refreshes the pages that did emit and withholds all destructive steps: deletion sweeps and sidebar/breadcrumb/llms/changelog rewrites that would delist pages kept on disk. A transient typo must never look like intentional removal.
...
- **Anchors are predicted, never written.** VitePress mints every heading id and `mintAnchors` predicts it: it walks `pageSteps`, the single description of a page's order shared with the page renderer, and applies markdown-it-anchor's rule via `headingSlug` in `src/markdown.ts` (ten lines vendored from `@mdit-vue/shared`). No element inside a heading may introduce extra text into the generated slug. Member names are wrapped in code spans (which `mintAnchors` slugifies), badges and source links are self-closing, and re-export badge links carry `class="ignore-header"`. Getting this wrong is **silent**, so `test/anchors.test.ts` renders every page through the real VitePress and asserts both directions.
...
- **A generated site's dependencies are one list.** `SITE_DEPENDENCIES` (`src/build.ts`) names them; `docsPackageJson` resolves their version ranges from this package's own `devDependencies`, and `ensureVitepress` installs when any of them fails to resolve. Adding one anywhere else means a site that resolves VitePress, skips the install, and then fails on a missing import during configuration loading.
- **Lute is pinned exactly too, and `src/lute.ts` is the only place that says so.** It holds the version, the per-platform asset names, and their sha256 digests, and it is what every spawn resolves through. Bumping it means replacing all four digests (the release assets carry their own `digest` field) and rerunning `regen:docmodels`, since test captures reflect the output of that specific Lute version. Nothing else may reach for a `lute` on the PATH: that would be an unpinned second version, exactly the drift the pin exists to prevent.

## Tests

Test coverage is organized across four layers. Place new coverage in the narrowest layer appropriate for the change:
